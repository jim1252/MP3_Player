...
def add_song():
	...
	song = song.replace("/home/pi/tkinter/MP3_Player/audio/", "")
...
def shuffle_song(): # not yet developed
	# Reset Slider and Status bar
	status_bar.config(text='')
	my_slider.config(value=0)
	# Select random song
...
# Add Multiple songs to playlist
add_song_menu.add_command(label="Add multipule Songs to playlist", command=add_multiple)
...
#current_song = Label(frame_song_display, text="")
#current_song.pack()
...
play_button = Button(control_frame, image=play_img, borderwidth=0, command=play)
